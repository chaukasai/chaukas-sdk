...
class MonkeyPatcher:
    ...
    def _apply_patch(self, patch: PatchTarget) -> None:
        ...
        try:
            ...
            if patch.condition and not patch.condition():
                logger.debug(f"Condition not met for {patch.module_name}.{patch.class_name}.{patch.method_name}")
                return
            ...
            logger.debug(f"Patched {patch.module_name}.{patch.class_name}.{patch.method_name}")
            
        except Exception as e:
            logger.error(f"Failed to patch {patch.module_name}.{patch.class_name}.{patch.method_name}: {e}")
    ...
    def _unpatch_target(self, patch: PatchTarget) -> None:
        ...
        try:
            ...
            logger.debug(f"Unpatched {patch.module_name}.{patch.class_name}.{patch.method_name}")
            
        except Exception as e:
            logger.error(f"Failed to unpatch {patch.module_name}.{patch.class_name}.{patch.method_name}: {e}")
...
def _serialize_value(value) -> Any:
    ...
    if value is None or isinstance(value, (str, int, float, bool)):
        ...
        return [_serialize_value(item) for item in value[:10]]  # Limit to first 10 items
    elif isinstance(value, dict):
        return {k: _serialize_value(v) for k, v in list(value.items())[:10]}  # Limit to first 10 items
    else:
