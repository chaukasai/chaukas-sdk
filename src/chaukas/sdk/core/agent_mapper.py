...
class AgentMapper:
    ...
    def map_custom_agent(
        ...
        if id_extractor:
            ...
            agent_id = (
                ...
                getattr(agent, "_id", None) or
                getattr(agent, "agent_id", None) or
                getattr(agent, "name", None) or
                str(id(agent))  # Last resort: Python object ID
            )
        ...
        if name_extractor:
            ...
            agent_name = (
                getattr(agent, "name", None) or
                getattr(agent, "agent_name", None) or
                getattr(agent, "role", None) or
                getattr(agent, "title", None) or
                "unknown"
            )
    ...
    @staticmethod
    def detect_and_map(agent: Any, framework_hint: Optional[str] = None, **kwargs) -> Tuple[str, str]:
        """
        ...
        module_name = agent.__class__.__module__.lower() if hasattr(agent.__class__, "__module__") else ""
        ...
        if hasattr(agent, "_id") or (hasattr(agent, "name") and hasattr(agent, "model")):
        ...
        return AgentMapper.map_custom_agent(
            agent,
            kwargs.get("id_extractor"),
            kwargs.get("name_extractor")
        )
...
def get_agent_info(agent: Any, framework: Optional[str] = None, **kwargs) -> Dict[str, str]:
