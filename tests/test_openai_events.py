...
class TestOpenAIAgentsEnhanced:
    ...
    async def test_tool_call_tracking(self):
        ...
        tool_event = next(e for e in events_sent if e.type == "EVENT_TYPE_TOOL_CALL_START")
    ...
    async def test_input_output_events(self):
        ...
        output_events = [e for e in events_sent if e.type == "EVENT_TYPE_OUTPUT_EMITTED"]
    ...
    async def test_model_invocation_events(self):
        ...
        start_events = [e for e in events_sent if e.type == "EVENT_TYPE_MODEL_INVOCATION_START"]
        end_events = [e for e in events_sent if e.type == "EVENT_TYPE_MODEL_INVOCATION_END"]
    ...
    async def test_error_handling_clears_pairs(self):
        ...
        assert self.wrapper.event_pair_manager.get_span_id_for_end("AGENT", "test-agent") is None
    ...
    async def test_session_cleanup_on_deletion(self):
        ...
        session_starts = [e for e in events_sent if e.type == "EVENT_TYPE_SESSION_START"]
