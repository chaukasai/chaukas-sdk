...
from chaukas.spec.common.v1.events_pb2 import EventType, EventStatus
...
class SpanHierarchyValidator:
    ...
    def __init__(self):
        ...
        self.start_end_pairs = defaultdict(dict)  # event_type -> {identifier -> {"start": event, "end": event}}
    ...
    def _track_start_end_pair(self, event):
        ...
        if event_type in start_end_map:
            ...
            self.start_end_pairs[event_type][pair_key] = {
                "start": event,
                "end": None
            }
    ...
    def _get_pair_key(self, event) -> str:
        ...
        if event.type in [EventType.EVENT_TYPE_SESSION_START, EventType.EVENT_TYPE_SESSION_END]:
            return event.session_id
        
        # Use agent_id for AGENT events
        if event.type in [EventType.EVENT_TYPE_AGENT_START, EventType.EVENT_TYPE_AGENT_END]:
            return event.agent_id
        
        # Use a combination for MODEL_INVOCATION events
        if event.type in [EventType.EVENT_TYPE_MODEL_INVOCATION_START, EventType.EVENT_TYPE_MODEL_INVOCATION_END]:
        ...
        if event.type == EventType.EVENT_TYPE_TOOL_CALL_END:
            return event.tool_response.tool_call_id if event.tool_response.tool_call_id else "unknown"
        
        # Use server name for MCP_CALL events
        if event.type in [EventType.EVENT_TYPE_MCP_CALL_START, EventType.EVENT_TYPE_MCP_CALL_END]:
        ...
        for event_type, pairs in self.start_end_pairs.items():
            for pair_key, pair_events in pairs.items():
                ...
                if start_event and end_event:
                    ...
                    errors.append(f"Missing END event for {EventType.Name(event_type)} (key: {pair_key})")
    ...
    def get_summary(self) -> Dict:
        ...
        return {
            ...
            "start_end_pairs": sum(len(pairs) for pairs in self.start_end_pairs.values()),
        }
...
def test_session_events_have_no_parent(builder):
    ...
    try:
        ...
        assert session_start.parent_span_id == "", "SESSION_START should not have parent_span_id"
        
        session_end = builder.create_session_end(session_id="test-session")
        assert session_end.parent_span_id == "", "SESSION_END should not have parent_span_id"
    finally:
        _parent_span_id.reset(token)
...
def test_missing_end_event_detection(builder):
    ...
    agent_start = builder.create_agent_start(agent_id="incomplete-agent", agent_name="Test")
    validator.add_event(agent_start)
