...
class EventHierarchyAnalyzer:
            ...
            for parent in list(orphaned_parents)[:5]:  # Show max 5
                ...
                for event in self.events:
                    if event.get('parent_span_id') == parent:
                        child_events.append(event.get('type', 'UNKNOWN').replace('EVENT_TYPE_', ''))
                self.inconsistencies.append(
        ...
        for span_id in self.span_map:
            if span_id not in visited:
                if has_cycle(span_id, visited, set()):
                    self.inconsistencies.append(f"Circular dependency detected involving span: {span_id[:12]}...")
                    break
    ...
    def generate_ascii_tree(self) -> str:
        ...
        def print_tree(span_id: str, indent: int = 0, is_last: bool = True, parent_prefix: str = "") -> None:
            ...
            for i, child_span in enumerate(unique_children):
                ...
                print_tree(child_span, indent + 1, is_last_child, 
        ...
        if session_spans:
            output.append(f"SESSION [{self.session_id[:12] if self.session_id else 'unknown'}...]")
    ...
    def write_debug_report(self) -> None:
        ...
        with open(debug_file, 'w') as f:
            ...
            checks = [
                ("Session ID Consistency", len(set(e.get('session_id') for e in self.events)) <= 1),
                ("Trace ID Consistency", len(set(e.get('trace_id') for e in self.events)) <= 1),
                ("No Orphaned Parents", len([p for p in self.parent_child_map if p not in self.span_map]) == 0),
            ...
            for event in sorted_events[:50]:  # Show first 50 events
                ...
                parent = event.get('parent_span_id', 'none')[:12] if event.get('parent_span_id') else 'none'
                f.write(f"{timestamp} | {event_type:30} | span:{span_id} | parent:{parent}\n")
            ...
            if not self.inconsistencies and not self.warnings:
                ...
                if self.warnings:
                    ...
                    if any('Unpaired' in w for w in self.warnings):
                        f.write("   - Ensure all START events have corresponding END events\n")
