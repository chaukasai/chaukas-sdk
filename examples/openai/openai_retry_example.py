...
class MockToolCall:
    ...
    def __init__(self, tool_name: str, arguments: Dict):
        ...
        self.function = type(
            "obj", (object,), {"name": tool_name, "arguments": json.dumps(arguments)}
        )()
    ...
    def __init__(self, content: str = None, tool_calls: List = None):
        ...
        self.choices = [
            type(
                ...
                {
                    "message": type(
                        "obj", (object,), {"content": content, "tool_calls": tool_calls}
                    )()
                },
            )()
        ]
...
class MockRunner:
    ...
    async def run_once(self, messages: List[Dict]) -> MockResponse:
        ...
        if "search" in content.lower():
            ...
            tool_calls = [MockToolCall("calculator", {"expression": content})]
            return MockResponse(tool_calls=tool_calls)

        # Regular response
        return MockResponse(
            content=f"I understand you said: '{content}'. Here's my response based on that input."
        )
...
async def main():
    ...
    test_messages = [
        {
            "role": "user",
            "content": "Hello! Can you help me search for information about Python?",
        },
    ...
    for i, message in enumerate(test_messages, 1):
        ...
        try:
            ...
            if result.content:
                ...
                print(
                    f"✓ Tool calls requested: {[tc.function.name for tc in result.tool_calls]}"
                )
        ...
        if retry_events:
            ...
            for retry in retry_events:
                if "retry" in retry:
                    r = retry["retry"]
                    print(
                        f"  - Attempt {r.get('attempt')}: {r.get('error_message', '')[:50]}..."
                    )
                    print(
                        f"    Strategy: {r.get('strategy')}, Backoff: {r.get('backoff_ms')}ms"
                    )
        else:
